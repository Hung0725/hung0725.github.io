...
function newSession(sessionName, ch, manual){
    ...
    newEl.value = session;
    newEl.setAttribute('onclick', 'Y = d.getFullYear(); console.log(this.value); session = parseInt(this.value); stat(); cur(Y); focusBtn(this); todayCount()');
    ...
    localStorage.edata = JSON.stringify(data); 
...
function cur(year){
    curr.innerHTML = year;
    if(data[session][year] == undefined){
        renderblank(year);
        return;
...
function sav(ID, val){
    ...
    if(!(val == 0 || val == undefined || val == NaN)){
        ...
        //console.log(em.style.background);
...
function stat(){
    ...
    var ac = 0, tol = 0, ch = false;
    var sum = 0;
    while(data[session][i] == undefined) i++;
    while(data[session][i] != undefined){
        var temp = new Date(), tmp = checkLeap(i);
        if(i == temp.getFullYear()) tmp = getDayOfYear(); 
        //console.log(tmp);
        for(var j = 1; j <= tmp; j++){
    ...
    var cSt = 0;
    console.log(ac , tol);
    var j = getDayOfYear(), i = temp.getFullYear();
    ...
    if(cSt > 0){
        inf[0].parentElement.children[0].children[0].classList.add('big-fire');
        inf[0].parentElement.children[0].children[0].classList.remove('no-fire');
    } else {
        inf[0].parentElement.children[0].children[0].classList.remove('big-fire');
        inf[0].parentElement.children[0].children[0].classList.add('no-fire');
    }
    ...
    console.log(sum);
    var actd = Math.floor(ac / tol * 10000) / 100 + '%';
    if(actd == "NaN%") actd = "0%";
    inf[5].innerText = actd;
    var avg = Math.floor(s / tol * 100) / 100;
    inf[6].innerText = avg;
}
...
ad.onkeydown = function(e){
    if(e.key == 'Enter'){
        addval(parseInt(ad.value));
        ad.value = '';
    }
}
