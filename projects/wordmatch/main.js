...
//
var wordtext;
if(localStorage.getItem('gcontent') == null){
    wordtext = "茶碗-noun-Rice bowl, tea cup/本屋-noun-Bookstore/服-noun-Chothes/書く-verb-Write/玄関-noun-Entrance, gate/宿題-noun-Homework/外国-noun-Foreign Country/銀行-noun-Bank/牛肉-noun-Beef/牛乳-noun-Cow's milk/台所-noun-Kitchen/調べる-verb-Investigate/動物-noun-Animal/電子辞書-noun-Electric dictionary/小学校-noun-Elementary school/明後日-noun-Day after tomorrow/夏休み-noun-Summer break/冬-noun-Winter/春-noun-Spring/夏-noun-Summer/秋-noun-Autumn/絵-noun-Picture/";
} else {
...
function findword(val){
    for(var i = 0; i < splitword.length; i++){
        var spl = splitword[i].split('-');
        if(val == spl[0]) return i;
    }
}
function findmeaning(val){
    for(var i = 0; i < splitword.length; i++){
        var spl = splitword[i].split('-');
        if(val == spl[2]) return i;
    }
}
