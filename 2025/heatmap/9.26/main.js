...
function newSession(sessionName, ch, manual){
    ...
    newEl.value = session;
    newEl.setAttribute('onclick', 'console.log(this.value); session = parseInt(this.value); stat(); cur(Y); focusBtn(this); todayCount()');
...
if(localStorage.cdata != undefined){
...
function stat(){
    ...
    while(data[session][i] == undefined || Object.keys(data[session][i]).length == 0) i++;
    while(data[session][i] != undefined && Object.keys(data[session][i]).length != 0){
    ...
    var actd = (ac + "/" + tol) + " (" + Math.floor(ac / tol * 10000) / 100 + '%)';
    if(actd == "NaN%") actd = "0%";
