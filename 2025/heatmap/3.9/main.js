...
function sav(ID, val){
    ...
    if(!(val == 0 || val == undefined || val == NaN)){
        ...
        if(!dark){
            ...
            var cr = color[colorOpt].length - Math.floor(val / threshold) - 1;
            if(cr < 0) cr = 0;
            if(cr > color[colorOpt].length - 2) cr = color[colorOpt].length - 2;
            var temp = cr;
            if(!temp) temp = 1;
...
function stat(){
    ...
    if(cSt > 0){
        inf[0].parentElement.children[0].classList.add('big-fire');
        inf[0].parentElement.children[0].classList.remove('no-fire');
    } else {
        inf[0].parentElement.children[0].classList.remove('big-fire');
        inf[0].parentElement.children[0].classList.add('no-fire');
    }
